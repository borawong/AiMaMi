...
本文件固定后续执行顺序，防止压缩、摘要或局部文档把任务顺序改偏。
...
1. 先保留两份原文文档，并在后续所有重构中以原文为验收标准。
2. 再调整目录结构，使前端和后端都能承载原文中的 owner 边界。
3. 再把原始前后端按照两份原文要求重构掉；现有代码也必须纳入重构，不因为“不新增”而保留旧边界。
4. 再按目标要求做前端全量还原：前端文件、路由、模块、入口、运行时、服务门面、缓存、弹窗、面板、测试和文案必须逐步对齐 raw/internal 全链条材料。
...
- 禁止把前端要求写成泛泛“主流模块化骨架”后停止。
- 禁止把后端写成“未还原所以空目录”；后端必须是真实六边形架构骨架。
- 禁止把现有前后端代码排除在重构外；现有代码也要按原文边界迁移。
- 禁止在仓库文档、注释、提交说明和还原说明中写入外部参考仓库名称。
- 禁止在仓库文档、注释、提交说明和还原说明中写入机器、用户、网络位置或内部来源标识。
- 所有文档和注释必须使用中文；用户可见文案必须按 locale 规则同步。
