...
`evidence/full-chain/internal/frontend-copy-acceptance.json` 当前已经存在，但状态是 `draft`。它只列出 950 个 locale key 的逐条验收槽位，并记录 raw/internal 中能证明 key 出现过的证据；它没有把任何条目标成验收通过。
...
- `scripts/generate-frontend-copy-acceptance.mjs` 生成全文案验收 draft；它不把 draft 当作 accepted。
...
- `frontend-copy-acceptance.json` 仍是 draft，所有 locale key 都缺少 raw/internal 中文和英文文案来源。
