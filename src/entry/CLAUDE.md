...
- 本目录只放应用组合入口、全局 Provider 装配、Router 装配、错误边界、runtime initializer 和 prompt host。
- 不得承载业务流程、文件状态、IPC 编排、模块私有状态或用户可见业务文案。
- Provider 顺序、QueryClient 实例和全局 error boundary 必须稳定，不能在页面切换时重建。
