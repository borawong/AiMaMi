...
function panelLoading(payload: unknown, query: PanelQueryState) {
  return !payload && (query.isPending || query.isFetching);
}

function panelError(payload: unknown, query: PanelQueryState, t: Translate) {
  return !payload && query.isError ? formatInvokeError(query.error, t("common.error")) : null;
}
