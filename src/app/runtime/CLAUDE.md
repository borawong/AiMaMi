...
## runtime initializer 边界

- initializer 必须是 null-render 或 hook 形式，只写 query/cache 或调度模块 action。
- 事件 payload 到 query key/cache helper 的映射必须集中声明，并集中清理监听。
- 事件重放、乱序和晚返回必须服从 mutation response 与最新 query sequence，不得直接写组件 state。
