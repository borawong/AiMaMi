...
- Router 只消费 route registry，不重复维护 route union、导航文案 key、可见性规则或 preload 规则。
- 页面和 layout 通过统一 lazy wrapper 管理 loading 与 chunk error。
- 不得在 Router 中写业务列表状态、弹窗状态机或模块数据请求。
