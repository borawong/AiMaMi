...
- 本目录只组合 providers、router、runtime initializer 和启动期辅助。
- 不得读取模块私有数据，不得渲染业务页面内容，不得绕过 route registry 维护导航状态。
- 桌面 basename、启动错误和调试入口只在 entry/app 组合边界处理。
