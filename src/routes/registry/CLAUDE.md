...
- 本目录是 route meta、layout、error boundary、visibility、preload、skeleton 和高 IO feedback 的唯一 owner。
- 新增 route 必须同步 AppRoute union、locale key、懒加载入口、可见性和 skeleton 规则。
- 不得在 registry 中写模块数据请求、弹窗状态机或业务列表状态。
