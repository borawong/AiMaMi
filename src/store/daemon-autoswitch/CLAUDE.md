# daemon-autoswitch store Claude 执行规则
...
## daemon-autoswitch store 边界

- 本目录只保存 daemon-autoswitch 的短生命周期 UI 状态、选择、草稿、hover/focus 和弹窗开关。
- 后端事实必须由 daemon-autoswitch query hook/cache helper owning，不得长期写入 store。
