...
- store 只保存模块 UI 状态、选择、草稿、hover/focus、弹窗开关和短生命周期 view model。
- 账号列表、runtime snapshot、配额历史、relay state、MCP、skills、custom-instructions 等后端事实不得长期存在 store 私有状态。
- URL 参数同步到模块状态必须放 hydration / updater 组件，不放展示组件。
