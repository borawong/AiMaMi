...
- src/lib/api.ts 是前端 service/API 门面的唯一公共入口之一，底层 transport 必须在窄门面内收口。
- desktop bridge、路径打开、更新器、托盘和系统能力必须先收敛为窄适配，再被模块 service 调用。
- 不得在 lib 中写模块私有状态机或页面流程。
