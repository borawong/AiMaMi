...
- 前端访问后端能力必须经 src/lib/api.ts 和模块 service wrapper 收口。
- commands.ts 必须与仓库内 frontend-contract-report.md 和 Rust invoke handler 覆盖关系一致。
- 新增或删除 command 必须同时更新 E2E mock、Rust command、API wrapper 和验证脚本。
