...
- 模块 service wrapper 是页面、组件、hooks 访问后端能力的唯一模块入口。
- service 可以调用 src/lib/api.ts 或窄 desktop adapter，不得直接暴露底层 transport 给组件。
- service 不保存长期后端事实；后端事实由模块 query hook/cache helper owning。
