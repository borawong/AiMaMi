...
- 本目录只负责 Tauri builder、state 注入、invoke handler、生命周期和窗口/托盘适配。
- invoke handler 必须注册 evidence-backed IPC command，不能遗漏前端契约中的 command。
- lifecycle 不写业务事务、文件 IO 事务、HTTP 或产品状态机。
