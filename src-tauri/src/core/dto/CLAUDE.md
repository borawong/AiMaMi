...
- 本目录只放 core 内部 DTO 转换与语义边界，不等同于 IPC DTO 或前端 TypeScript 类型。
- 改 DTO 必须同步 Rust IPC DTO、前端 TS types、API wrapper 和 E2E mock。
- DTO 字段必须来自仓库内证据或显式骨架占位。
