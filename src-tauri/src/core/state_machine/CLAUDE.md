# Rust state machine Claude 执行规则
...
- 本目录只放领域状态机、状态转移和并发保护语义。
- single-flight、stale/delayed response 和事件重放语义必须保留诊断 code 和脱敏错误。
- 不得把 UI busy、按钮 disabled 或 toast 文案当作状态机事实。
