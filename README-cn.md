...
| **SSH 远端同步** | 将本地 `~/.codex` Codex 环境镜像到 SSH 服务器，并打开已准备好的远端会话 |

**智能路由说明：** 中转模型经 AiMaMi 本地代理转发，使用期间需保持 AiMaMi 运行。

**SSH 远端同步说明：** 远端服务器默认使用本机 OpenSSH 配置与 `ssh-agent`。AiMaMi 只保存主机元数据和可选密钥路径，不保存密码或私钥内容。远端同步要求本机和远端都安装 `rsync`，会把 Codex 文件镜像到配置的远端 `CODEX_HOME`，并排除 AiMaMi 本机临时状态和日志。
